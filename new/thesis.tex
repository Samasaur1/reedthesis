...
% - `disabletocchapterdots' - disable the Table of Contents dots between chapters.
% - `disabletocchaptername' - shows just "N" instead of "Chapter N" in each ToC entry.
% - `headeruppercase' - displays header titles in upper case (upper case takes priority over small caps)
% - `headersmallcaps` - displays header titles in small caps.
% - `coloredlinks` - displays monotone (blue) colored links.
%	- If you go to print, and links don't show up, check your engine! This should work fine on Overleaf, but you may need to
%		change which LaTeX engine you use (e.g. for LaTeX Workshop on Visual Studio Code, use the recipe `pdflatex')
% - `multicoloredlinks` - displays colorful colored links - `coloredlinks` is preferred.
...
% if you're using biblatex, the next line forces every entry in the biblatex file to be included
